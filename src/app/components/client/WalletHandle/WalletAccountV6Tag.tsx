...
export default function WalletAccountV6Tag() {
  ...
  const tx404 = useMemo(() => {
    if (!myWalletAccount || !isStrk20Network) return undefined;
    return createTx404({
      walletAccount: myWalletAccount,
      walletApiVersions,
      network: myFrontendProviderIndex === 0 ? "mainnet" : "sepolia",
      walletNetwork: chain
      ...
      confirmation: { timeoutMs: 20 * 60_000, pollIntervalMs: 3_000 },
    });
  }, [chain, isStrk20Network, myFrontendProviderIndex, myWalletAccount, walletApiVersions]);
  const privacySupported = tx404?.getCapabilities().privacy ?? false;
  ...
  async function submit(
    request: () => Promise<Tx404Submission>,
    setResult: (r: ActionResult) => void,
    amountLabel: string
  ): Promise<string | undefined> {
    if (!tx404) {
      setResult(errorResult("No Tx404 client available for this wallet and network."));
      return undefined;
    }
    let submission: Tx404Submission;
    try {
    ...
    const txH = submission.transactionHash;
    setResult({
      status: "pending",
    ...
    try {
      const confirmation = await submission.wait();
      if (confirmation.status === "confirmed") {
        setResult(receiptToResult(confirmation.receipt, txH, amountLabel));
      } else {
        setResult({
          status: "pending",
          title: "Transaction submitted",
          rows: [{ label: "Transaction", value: shortHex(txH), hash: txH }],
          note: "Confirmation is still pending. Use the explorer link to follow it.",
        });
      }
    } catch (error: any) {
      setResult({
        status: "pending",
        title: "Transaction submitted; confirmation timed out",
        rows: [{ label: "Transaction", value: shortHex(txH), hash: txH }],
  ...
  const handleBalances = async () => {
    ...
    if (!tx404) {
      setResultBalances(errorResult("No Tx404 client available."));
      return;
    }
    try {
      const r = await tx404.getBalances({ tokens: [] });
  ...
  const handleShield = async () => {
    ...
    await submit(() => tx404!.shield({ token: TOKEN, amount: TEN_STRK }), setResultShield, "10 STRK");
  };
  ...
  const handleUnshield = async () => {
    ...
    await submit(
      () => tx404!.unshield({ token: TOKEN, amount: ONE_STRK, recipient: connectedAddress }),
      setResultUnshield,
      "1 STRK"
    );
  };
  ...
  const handleSelfTransfer = async () => {
    ...
    await submit(
      () => tx404!.pay({ token: TOKEN, amount: ONE_STRK, recipient: connectedAddress, reference: "demo-self-transfer" }),
      setResultTransfer,
      "1 STRK"
    );
  };
  ...
  const handleComplex = async () => {
    ...
    const txH = await submit(
      () => tx404!.invoke({ actions, disclosure: DISCLOSURES.invoke }),
      setResultComplex,
      "5 STRK"
    );
    if (!txH) return;
  ...
  > = {
    shield: { label: "You're shielding", value: "10", token: "STRK", hint: "Deposit into the privacy pool", cta: "Shield", onRun: handleShield, result: resultShield, disabled: !isStrk20Network || !privacySupported },
    send: { label: "You're sending - to self", value: "1", token: "STRK", hint: "Private in-pool transfer", cta: "Self transfer", onRun: handleSelfTransfer, result: resultTransfer, disabled: !isStrk20Network || !privacySupported },
    unshield: { label: "You're unshielding", value: "1", token: "STRK", hint: "Withdraw to your account", cta: "Unshield", onRun: handleUnshield, result: resultUnshield, disabled: !isStrk20Network || !privacySupported },
    echo: { label: "Echo invoke round-trip", value: "5", token: "STRK", hint: "Withdraw → helper → refill open note", cta: "Run echo", onRun: handleComplex, result: resultComplex, disabled: !isStrk20Network || !privacySupported || !hasEchoHelper },
